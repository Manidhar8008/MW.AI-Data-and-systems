...
class FabricationRules:
    ...
    mullion_allowance_mm: Decimal = Decimal("6")
...
class UPVCFabricationPlanner:
    ...
    The dimensional offsets are intentionally generic and configurable. Real
    production must replace them with the fabricator's approved system, sash,
    glass, hardware and reinforcement rules.
    """
    ...
    def build_glass_dimensions(self, configuration: UPVCConfiguration) -> tuple[GlassPiece, ...]:
        ...
        glass_width = sash_width - r.glass_clearance_mm
        glass_height = mm(m.height_mm) - r.glass_clearance_mm
        if glass_width <= 0 or glass_height <= 0:
            raise ValueError("Calculated glass dimensions are not positive")
        return tuple(
            GlassPiece(
                code=f"GLASS-{configuration.glass.value.upper()}",
                width_mm=mm(glass_width),
                height_mm=mm(glass_height),
                quantity=configuration.sash_count,
            ),
        )
    ...
    def build_reinforcement_schedule(self, configuration: UPVCConfiguration, profile_cuts: Iterable[CutPiece]) -> tuple[CutPiece, ...]:
        ...
        for piece in profile_cuts:
            if piece.material is CutMaterial.PROFILE and piece.length_mm >= threshold:
                output.append(
                    CutPiece(
                        material=CutMaterial.REINFORCEMENT,
                        code=self.rules.reinforcement_code,
                        length_mm=piece.length_mm,
                        quantity=piece.quantity,
                        source=f"reinforce:{piece.source}",
                    )
                )
        return tuple(output)
    ...
    def build_cut_plan(self, pieces: Iterable[CutPiece], stock_length_mm: Decimal) -> CutPlan:
        expanded: list[CutPiece] = []
        for piece in pieces:
            expanded.extend(
                CutPiece(piece.material, piece.code, piece.length_mm, 1, piece.source)
        ...
        for piece in expanded:
            ...
            for index, rem in enumerate(remaining):
                if piece.length_mm <= rem:
                    stocks[index].append(piece.length_mm)
                    remaining[index] = rem - piece.length_mm
                    placed = True
                    break
            if not placed:
                stocks.append([piece.length_mm])
                remaining.append(stock_length_mm - piece.length_mm)

        stock_rows = tuple(
            StockCut(stock_length_mm, tuple(cuts), remaining[index])
            for index, cuts in enumerate(stocks)
        )
        piece_length = sum((item.length_mm for item in expanded), Decimal("0"))
        stock_length = stock_length_mm * Decimal(len(stock_rows))
        waste = stock_length - piece_length
        utilization = (piece_length / stock_length * Decimal("100")) if stock_length else Decimal("0")

        return CutPlan(
            material=expanded[0].material if expanded else CutMaterial.PROFILE,
            stock_length_mm=stock_length_mm,
            pieces=tuple(expanded),
            stocks=stock_rows,
            total_piece_length_mm=mm(piece_length),
            total_stock_length_mm=mm(stock_length),
            waste_mm=mm(waste),
    ...
    def plan(self, configuration: UPVCConfiguration, order_id: str) -> ProductionOrder:
        ...
        profile_plan = self.build_cut_plan(profile_cuts, self.rules.profile.stock_length_mm)
        reinforcement_plan = self.build_cut_plan(reinforcement, self.rules.reinforcement_stock_length_mm)
        ...
        return ProductionOrder(
            ...
            reinforcement=reinforcement,
            hardware=hardware,
            profile_cuts=profile_plan,
            reinforcement_cuts=reinforcement_plan,
            warnings=warnings,
        )
